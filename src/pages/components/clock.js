...
const Clock = forwardRef(function Clock({ style, spins = 2, duration = 1.5, className, isMobile = false }, ref) {
  ...
  return (
    <div className={className} style={isMobile ? { position: 'relative', width: 120, height: 120, ...style } : { position: 'absolute', width: 200, height: 200, ...style }}>
      {!isMobile && (
        <>
          ...
          <canvas
            ...
            style={{ position: 'absolute', right: '50%', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', zIndex: 0 }}
          />
        </>
      )}
      ...
      <div
        style={isMobile ? {
          position: 'absolute', top: '50%', left: '50%', width: '35%', height: '3px', marginTop: '-1.5px', filter: 'drop-shadow(-2px 2px 0px #d5c2bf)', zIndex: 2
        } : {
          position: 'absolute', top: '50%', left: '50%', width: '45px', height: '3px', marginTop: '-1.5px', filter: 'drop-shadow(-2px 2px 0px #d5c2bf)', zIndex: 2
        }}
      >
        <motion.div
          style={{ position: 'absolute', width: '70%', height: '100%', backgroundColor: isMobile ? '#333' : 'white', transformOrigin: 'left center', rotate: hourRotate }}
        />
        <motion.div
          style={{ position: 'absolute', width: '100%', height: '100%', backgroundColor: isMobile ? '#555' : 'white', transformOrigin: 'left center', rotate: minuteRotate }}
        />
        <motion.div
          style={{ position: 'absolute', width: '100%', height: '100%', backgroundColor: 'red', transformOrigin: 'left center', rotate: secondRotate }}
        />
      </div>

      <div
        style={{
          position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 6, height: 6, backgroundColor: 'black', zIndex: 3, borderRadius: isMobile ? '50%' : '30%'
        }}
      />
    </div>
  );
